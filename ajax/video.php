    ...
    if(mysqli_num_rows($query) > 0){
        foreach($query as $row){
            ...
            ?>
               ...
               <div class="flex-grow-1 ms-3">
                     ...
                     <div class="hstack align-items-center mb-0" style="margin-left:-.25rem;">
                        ...
                        if(isset($_SESSION['loggedOren'])){
                            ?>
                            <button class="btn btn-sm btn-secondary rounded-pill small collapsed" value='<?= $_SESSION['loggedOren']['unique_id'] ?>'  data-bs-toggle="collapse" data-bs-target="#<?= $row['unique_id'] ?>r" aria-expanded="true" aria-controls="<?= $row['unique_id'] ?>r">Reply</button>
                            <?php
                            if($_SESSION['loggedOren']['unique_id'] == $row['author']){
                                ?>
                                    <button class="btn btn-sm  btn-danger rounded-pill small delete" style='margin-left:5px' value='<?= $row['unique_id'] ?>'>Delete</button>
                                <?php
                            } 
                        }
                            ?>
                        ...
                        <div class="comment-box mt-2 collapse" id="<?= $row['unique_id'] ?>r">
                            <div class="d-flex comment">
                                <?php if(isset($_SESSION['loggedOren'])) : ?>
                                    <h1 class="<?= $user['first_name'][0] ?>"><?= $user['first_name'][0] ?></h1>
                                <?php else :?>
                                    <h1 class="M">M</h1>
                                <?php endif; ?>
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <div class="hstack justify-content-end gap-1">
                                        <button class="cancel btn btn-sm btn-secondary rounded-pill collapsed" data-bs-toggle="collapse" data-bs-target="#<?= $row['unique_id'] ?>r" aria-expanded="true" aria-controls="<?= $row['unique_id'] ?>" >Cancel</button>
                                        <button class="btn btn-sm btn-primary rounded-pill add_reply" value ="<?= $row['unique_id'] ?>" >Reply</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                     ...
                     <div style="margin-left:-.769rem;">
                        <button class="btn rounded-pill d-inline-flex align-items-center collapsed" data-bs-toggle="collapse" data-bs-target="#<?= $row['unique_id'] ?>c" aria-expanded="true" aria-controls="<?= $row['unique_id'] ?>c">
               ...
               <div class="collapse reply_show" id="<?= $row['unique_id'] ?>c" style="">
                    ...
                    <div class="d-flex comment">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <div class="hstack align-items-center mb-0" style="margin-left:-.25rem;">
                                ...
                                if(isset($_SESSION['loggedOren'])){
                                    ?>
                                    <button class="btn btn-sm btn-secondary rounded-pill small collapsed"  data-bs-toggle="collapse" data-bs-target="#<?= $row['parent_id'] ?>r" aria-expanded="true" aria-controls="<?= $row['parent_id'] ?>r">Reply</button>
                                    <?php
                                    if($_SESSION['loggedOren']['unique_id'] == $row['author']){
                                        ?>
                                    <button class="btn btn-sm  btn-danger rounded-pill small delete" style='margin-left:5px' value='<?= $row['unique_id'] ?>'>Delete</button>
