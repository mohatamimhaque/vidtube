function video() {
    ...
    var fine_scrubbing = document.querySelector('.video_wrapper .fine_scribbing');
...
let h_first;
s_progress_area.addEventListener("touchstart", (e) => {
    h_first = e.offsetY;
...
function control() {
    ...
    let link = document.querySelector(".play_next");
    let kkk = '?video=' + contents[0].querySelector('.delete').value.trim();
    let href = location.href.split('?video=')[0];
    //alert(href);
    link.classList.add('active');
    link.setAttribute("href", `${href}${kkk}`);

    let previousLink = document.referrer;

    if (previousLink.includes('video.php')) {
        let play_previous = document.querySelector(".play_previous");

        ;
        play_previous.setAttribute("href", `${previousLink}`);
        // play_previous.style.display='none'
        play_previous.classList.add('active');
    ...
    for (let tt = 0; tt < contents.length; tt++) {
        ...
        main_video.addEventListener("loadeddata", () => {
            ...
            thumbnailVideo.src = main_video.src;
            thumbnailVideo.currentTime = main_video.duration * 0.3;
            canvas.width = main_video.videoWidth;
            canvas.height = main_video.videoHeight;
